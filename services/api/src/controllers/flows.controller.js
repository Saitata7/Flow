...
const getUserFlows = async (request, reply) => {
  ...
  try {
    ...
    const flows = await FlowModel.findByUserId(user.id, {
...
const searchFlows = async (request, reply) => {
  ...
  if (tags) {
    ...
    searchCriteria.push(flow => 
      flow.tags && tagList.some(tag => flow.tags.includes(tag))
    );
  }
  ...
  // Filter flows
  let searchResults = Array.from(flows.values()).filter(flow => {
...
const getFlowStats = async (request, reply) => {
  ...
  const stats = {
    ...
    completionRate: entries.length > 0 ? (entries.filter(entry => entry.symbol === '✓').length / entries.length) * 100 : 0,
  };
