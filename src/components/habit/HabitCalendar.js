...
const HabitCalendar = ({ habit, onUpdateStatus, onMonthChange, currentMonth }) => {
  ...
  useEffect(() => {
    console.log('Habit status updated:', habit.status);
  }, [habit.status]);

  console.log('Habit status received:', habit.status, 'trackingType:', habit.trackingType);
    ...
    if (habit.trackingType === 'Quantitative') {
      alertOptions.unshift(
        {
          ...
          onPress: () => {
            setModalState({
              ...
              note: '',
              selectedEmotion: null,
              quantitativeCount: (habit.status?.[dateKey]?.quantitative?.count || 0).toString(),
              timeDuration: { hours: '', minutes: '', seconds: '' },
            });
            setTimeout(() => { isUpdating.current = false; }, 100);
          },
        },
        {
          text: 'Not Set ➖',
          onPress: () => {
            onUpdateStatus(habit.id, dateKey, { symbol: '➖', quantitative: { count: 0, goal: habit.goal || 1, unitText: habit.unitText || '' } }, habit.status);
            setModalState({ selectedDate: null, pendingStatus: null, showResponseModal: false, modalStage: 'input', note: '', selectedEmotion: null, quantitativeCount: '', timeDuration: { hours: '', minutes: '', seconds: '' } });
            setTimeout(() => { isUpdating.current = false; }, 100);
          },
        }
      ...
      alertOptions.unshift(
        {
          text: 'Set Duration',
          onPress: () => {
            const totalDuration = habit.status?.[dateKey]?.timebased?.totalDuration || habit.status?.[dateKey]?.timeUpdate?.totalDuration || 0;
            setModalState({
              selectedDate: dateKey,
        ...
        },
        {
          text: 'Not Set ➖',
          onPress: () => {
            onUpdateStatus(habit.id, dateKey, { symbol: '➖', timebased: { totalDuration: 0, pausesCount: 0, hours: habit.hours || 0, minutes: habit.minutes || 0, seconds: habit.seconds || 0 } }, habit.status);
            setModalState({ selectedDate: null, pendingStatus: null, showResponseModal: false, modalStage: 'input', note: '', selectedEmotion: null, quantitativeCount: '', timeDuration: { hours: '', minutes: '', seconds: '' } });
            setTimeout(() => { isUpdating.current = false; }, 100);
          },
        }
      ...
      alertOptions.unshift(
        ...
        },
        {
          text: 'Not Set ➖',
          onPress: () => {
            onUpdateStatus(habit.id, dateKey, { symbol: '➖' }, habit.status);
            setModalState({ selectedDate: null, pendingStatus: null, showResponseModal: false, modalStage: 'input', note: '', selectedEmotion: null, quantitativeCount: '', timeDuration: { hours: '', minutes: '', seconds: '' } });
            setTimeout(() => { isUpdating.current = false; }, 100);
          },
        }
  ...
  }, [cheatMode, habit.id, habit.status, habit.trackingType, habit.goal, habit.unitText, habit.hours, habit.minutes, habit.seconds, onUpdateStatus]);
  ...
  const handleResponseSubmit = useCallback(() => {
    ...
    console.log('Submitting updates:', { id: habit.id, date: modalState.selectedDate, updates });
    onUpdateStatus(habit.id, modalState.selectedDate, updates, habit.status);
    setModalState({ selectedDate: null, pendingStatus: null, showResponseModal: false, modalStage: 'input', note: '', selectedEmotion: null, quantitativeCount: '', timeDuration: { hours: '', minutes: '', seconds: '' } });
    setTimeout(() => { isUpdating.current = false; }, 100);
  }, [modalState, habit.id, habit.trackingType, habit.goal, habit.unitText, habit.hours, habit.minutes, habit.seconds, onUpdateStatus]);
  ...
  const handleResponseSkip = useCallback(() => {
    ...
    console.log('Skipping note/emotion:', { id: habit.id, date: modalState.selectedDate, updates });
    onUpdateStatus(habit.id, modalState.selectedDate, updates, habit.status);
    setModalState({ selectedDate: null, pendingStatus: null, showResponseModal: false, modalStage: 'input', note: '', selectedEmotion: null, quantitativeCount: '', timeDuration: { hours: '', minutes: '', seconds: '' } });
    setTimeout(() => { isUpdating.current = false; }, 100);
  }, [modalState, habit.id, habit.trackingType, habit.goal, habit.unitText, habit.hours, habit.minutes, habit.seconds, onUpdateStatus]);
  ...
  const markedDates = useMemo(() => {
    const dates = {};
    Object.entries(habit.status || {}).forEach(([date, status]) => {
    ...
    return dates;
  }, [habit.status, habit.trackingType, habit.goal, highContrast]);

  const renderCustomDay = useCallback(({ date, state }) => {
    const dateKey = date.dateString;
    const status = habit.status?.[dateKey];
    ...
    if (habit.trackingType === 'Quantitative') {
      const count = status?.quantitative?.count || 0;
      const unitText = status?.quantitative?.unitText || habit.unitText || '';
  ...
  }, [habit.status, habit.trackingType, habit.unitText, theme, textSize, highContrast]);
  ...
  return (
    <>
      ...
      <ResponseModal
        ...
        title={`Update ${habit.title} for ${moment(modalState.selectedDate).format('MMMM D, YYYY')}`}
      ...
      >
        {modalState.modalStage === 'input' && habit.trackingType === 'Quantitative' && (
          <View style={dynamicStyles.inputContainer}>
            <View style={{ flex: 1 }}>
              <Text style={dynamicStyles.inputLabel}>Count ({habit.unitText || 'units'})</Text>
              <TextInput
        ...
        {modalState.modalStage === 'input' && habit.trackingType === 'Time-based' && (
          <View style={dynamicStyles.inputContainer}>
            <View style={{ flex: 1 }}>
